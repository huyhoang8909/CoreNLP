 ...
 * @author lainam
 */
public class CustomTokenizerAnnotator implements Annotator {

    public static final String EOL_PROPERTY = "tokenize.keepeol";
    /**
     * A logger for this class
     */
    private static final Redwood.RedwoodChannels log = Redwood.channels(CustomTokenizerAnnotator.class);
     ...
     */
    public CustomTokenizerAnnotator() {
        this(false);
    }

    // CONSTRUCTORS

    public CustomTokenizerAnnotator(Properties properties) {
        this(false, properties, computeExtraOptions(properties));
    }


    public CustomTokenizerAnnotator(boolean verbose) {
        this(verbose, TokenizerType.English);
    }


    public CustomTokenizerAnnotator(String lang) {
        this(true, lang, null);
    }

    public CustomTokenizerAnnotator(boolean verbose, TokenizerType lang) {
        this(verbose, lang.toString());
    }

    public CustomTokenizerAnnotator(boolean verbose, String lang) {
        this(verbose, lang, null);
    }

    public CustomTokenizerAnnotator(boolean verbose, String lang, String options) {
        this(verbose, lang == null ? null : PropertiesUtils.asProperties("tokenize.language", lang), options);
    }

    public CustomTokenizerAnnotator(boolean verbose, Properties props) {
        this(verbose, props, null);
    }

    public CustomTokenizerAnnotator(boolean verbose, Properties props, String options) {
        if (props == null) {
     ...
     * When adding a new Tokenizer, modify TokenizerType.getTokenizerType() to retrieve your tokenizer from the
     * properties file, and then add a class is the switch structure here to instantiate the new Tokenizer type.
     *
    ...
    /**
     * Does the actual work of splitting TextAnnotation into CoreLabels, which are then attached to the
     * TokensAnnotation.
     */
    @Override
    public void annotate(Annotation annotation) {
        ...
        if (annotation.containsKey(CoreAnnotations.TextAnnotation.class)) {
            ...
            // don't wrap in BufferedReader.  It gives you nothing for in-memory String unless you need the
            // readLine() method!
